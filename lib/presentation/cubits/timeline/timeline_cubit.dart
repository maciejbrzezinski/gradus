...
class TimelineCubit extends Cubit<TimelineState> {
  ...
  Future<void> transformNoteToTask({required String noteId, required String taskTitle}) async {
    ...
    // 1. Update state immediately
    if (currentState.getItemById(noteId) == null) {
      emit(currentState.copyWith(items: [...currentState.items, TimelineItem.task(task)]));
    } else {
      final updated = currentState.items.where((item) => item.id != noteId);
      emit(currentState.copyWith(items: [...updated, TimelineItem.task(task)]));
    }

    // 2. Save in background (fire-and-forget)
    _saveInBackground(() => _updateTimelineItemUseCase(TimelineItem.task(task)));
  }
  ...
  Future<void> transformTaskToNote({
    ...
    final now = DateTime.now();
    final note = Note(id: taskId, createdAt: now, updatedAt: now, content: noteContent, type: noteType);
    ...
    _saveInBackground(() => _updateTimelineItemUseCase(TimelineItem.note(note)));
  }
  ...
  Future<void> transformNoteType({required String noteId, required String content, required NoteType newType}) async {
    ...
    final now = DateTime.now();
    final note = Note(id: noteId, createdAt: now, updatedAt: now, content: content, type: newType);
